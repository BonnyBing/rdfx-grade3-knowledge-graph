...
/** 各视图的默认筛选条件，与页面 applyViewDefaults 保持一致 */
function viewDefaults(view, data, showAllDetail) {
  const overview = [...GraphCore.OVERVIEW_TYPES]
  if (view === 'math' || view === 'science') {
    return {
...
async function prepare(params = {}) {
  ...
  const defaults = { ...viewDefaults(view, data, detail), ...(viewOverrides || {}) }
  ...
  const built = profile.buildOption(core, model, {
    ...
    meta: {
      ...
      viewLabel:
        (profile.VIEW_LABELS[view] || view) + (detail ? '（含详细节点）' : ''),
