...
const LENSES = ['structure', 'progression', 'standards', 'cross']
...
function normalize(raw = {}) {
  let view = VIEWS.includes(raw.view) ? raw.view : DEFAULTS.view
  let lens = LENSES.includes(raw.lens) ? raw.lens : DEFAULTS.lens
  // 旧链接：view=cross → lens=cross + view=all
  if (view === 'cross') {
    lens = 'cross'
    view = 'all'
  }
...
function pageQuery(params) {
  ...
  if (p.lens !== DEFAULTS.lens) qs.set('lens', p.lens)
...
const SUBJECT_HINTS = [
  { view: 'cross', lens: 'cross', words: ['跨学科', '学科融合', '跨科'] },
  { view: 'math', words: ['数学', '算数', '运算'] },
  { view: 'science', words: ['科学', '自然'] },
]

const LENS_HINTS = [
  { lens: 'progression', words: ['学习进阶', '前置', '先学', '补学', '依赖'] },
  { lens: 'standards', words: ['课标对齐', '课标', '课程标准', '学业要求'] },
  { lens: 'cross', words: ['跨学科', '学科融合', '跨科'] },
  { lens: 'structure', words: ['教材结构', '单元结构', '目录'] },
]
...
function guessLens(text) {
  const t = normalizeText(text)
  for (const hint of LENS_HINTS) {
    if (hint.words.some((w) => t.includes(normalizeText(w)))) return hint.lens
  }
  return null
}
...
function subjectOf(id) {
  const bare = String(id || '').startsWith('G5') ? String(id).slice(2) : String(id || '')
...
function resolveQuery(text, graphData) {
  ...
  return {
    view: resolvedView === 'cross' ? 'all' : resolvedView || 'all',
    lens: lens || (resolvedView === 'cross' ? 'cross' : 'structure'),
    focus,
    matches,
