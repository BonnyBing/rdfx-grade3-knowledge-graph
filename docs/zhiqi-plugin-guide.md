...
面向智启智能体：用本插件替代「图谱知识库检索 + 静态图片索引」。接入**智启知识图谱**（当前首批为三年级上册数学与科学，后续扩展至全学段全学科）。
...
**重要：智启不要直连 Vercel。** 智启国内机房访问不了 `*.vercel.app`，会 30 秒超时。请按 [tencent-scf-proxy.md](tencent-scf-proxy.md) 建腾讯云**香港** Web 函数中转，OpenAPI 的 `servers.url` 填中转地址。
